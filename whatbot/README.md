<p align="center">
  <img src="docs/logo/whatbot_logo.webp" alt="whatbot Logo" width="200"/>
</p>
...
<p align="center">
  <a href="https://github.com/Zaidx-me/whatbot/actions/workflows/ci.yml"><img src="https://github.com/Zaidx-me/whatbot/actions/workflows/ci.yml/badge.svg?branch=main" alt="CI"/></a>
  <img src="https://img.shields.io/github/package-json/v/Zaidx-me/whatbot?label=version&color=blue" alt="Version"/>
...
**whatbot** is a free, open-source WhatsApp API Gateway designed for developers who need full control over their messaging infrastructure—without vendor lock-in or hidden paywalls.

Built on a **pluggable architecture**, whatbot lets you swap database engines (SQLite/PostgreSQL), storage backends (Local/S3), and cache layers (Memory/Redis) without changing a single line of application code.
...
```bash
# Clone and start
git clone https://github.com/Zaidx-me/whatbot.git
cd whatbot
...
dumb-init (PID 1)
  └─ docker-entrypoint.sh (root — fixes named-volume ownership via chown)
       └─ gosu whatbot node dist/main  (drops to the whatbot user)
```

- **dumb-init** is PID 1 and forwards signals (SIGTERM, etc.) for graceful shutdown.
- **docker-entrypoint.sh** runs as root only long enough to `chown` the named-volume mount points so the `whatbot` user can write to them.
...
Named volumes (e.g. `whatbot-data`) get their ownership corrected automatically on every start, so no manual `chown` step is needed after volume creation.
...
> needs no profile — it is always available wherever `whatbot-api` runs. For TLS/public exposure,
...
whatbot can expose a **curated set of tools over the [Model Context Protocol](https://modelcontextprotocol.io)** so AI agents (Claude, Cursor, …) can drive WhatsApp. It is **off by default** and **additive** — every REST route keeps working unchanged.
...
**whatbot** – Free, Open Source WhatsApp API Gateway

[📖 Documentation](./docs/README.md) · [🔌 API Docs](http://localhost:2785/api/docs) · [🐛 Report Bug](https://github.com/Zaidx-me/whatbot/issues) · [💡 Request Feature](https://github.com/Zaidx-me/whatbot/issues)

<br/>

<sub>Made with ❤️ by <a href="https://github.com/Zaidx-me">zaidxme</a> and the whatbot Community</sub>
