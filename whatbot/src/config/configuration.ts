export default () => ({
  ...
  dataDatabase: {
    ...
    database: process.env.DATABASE_NAME || './data/whatbot.sqlite',
    ...
    name: process.env.DATABASE_NAME || 'whatbot',
  ...
  plugins: {
    ...
    // Remote catalog of installable plugins (JSON array; the whatbot-plugins repo's plugins.json).
    // Fetched through the SSRF guard — add its host to SSRF_ALLOWED_HOSTS if it is not publicly resolvable.
    catalogUrl:
      process.env.PLUGIN_CATALOG_URL || 'https://raw.githubusercontent.com/Zaidx-me/whatbot-plugins/main/plugins.json',
