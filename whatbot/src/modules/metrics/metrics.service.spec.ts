...
describe('MetricsService', () => {
  ...
  describe('render', () => {
    it('emits Prometheus exposition with session + message gauges', async () => {
      ...
      expect(out).toContain('whatbot_up 1');
      expect(out).toContain('whatbot_sessions_active 2');
      expect(out).toContain('whatbot_sessions_total 3');
      expect(out).toContain('whatbot_sessions{status="ready"} 2');
      expect(out).toContain('whatbot_sessions{status="failed"} 1');
      expect(out).toContain('whatbot_messages_total{direction="outgoing"} 100');
      expect(out).toContain('whatbot_messages_total{direction="incoming"} 50');
      expect(out).toContain('whatbot_messages_failed_total 3');
      // Every metric must declare HELP/TYPE before its sample.
      expect(out).toContain('# TYPE whatbot_messages_total counter');
      expect(out.endsWith('\n')).toBe(true);
    });
