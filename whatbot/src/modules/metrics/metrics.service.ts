...
/**
 * Prometheus exposition for whatbot. Kept dependency-free (no prom-client) — the
...
export class MetricsService {
  ...
  async render(): Promise<string> {
    ...
    gauge('whatbot_up', 'Whether the whatbot process is up (always 1 when scraped).', 1);
    gauge('whatbot_process_uptime_seconds', 'Process uptime in seconds.', Math.round(process.uptime()));
    gauge('whatbot_process_resident_memory_bytes', 'Resident set size in bytes.', mem.rss);
    gauge('whatbot_process_heap_used_bytes', 'V8 heap used in bytes.', mem.heapUsed);

    gauge('whatbot_sessions_total', 'Total number of configured sessions.', overview.sessions.total);
    gauge('whatbot_sessions_active', 'Number of READY (active) sessions.', overview.sessions.active);

    // Per-status session counts share one metric name with a `status` label.
    lines.push('# HELP whatbot_sessions Number of sessions by status.');
    lines.push('# TYPE whatbot_sessions gauge');
    for (const [status, count] of Object.entries(overview.sessions.byStatus)) {
      lines.push(`whatbot_sessions{status="${this.escapeLabel(status)}"} ${count}`);
    }

    lines.push('# HELP whatbot_messages_total Total messages by direction.');
    lines.push('# TYPE whatbot_messages_total counter');
    lines.push(`whatbot_messages_total{direction="outgoing"} ${overview.messages.sent}`);
    lines.push(`whatbot_messages_total{direction="incoming"} ${overview.messages.received}`);

    lines.push('# HELP whatbot_messages_failed_total Total messages in FAILED state.');
    lines.push('# TYPE whatbot_messages_failed_total counter');
    lines.push(`whatbot_messages_failed_total ${overview.messages.failed}`);
