...
describe('DockerService.getRunningBuiltinServices', () => {
  const container = (name: string, service: string, state: string) => ({
    ...
    state,
    status: state,
    labels: { 'com.whatbot.service': service, 'com.whatbot.builtin': 'true' },
  });
  ...
  it('reports a service built-in only when its labeled container is actually running', async () => {
    ...
    jest
      ...
      .mockResolvedValue([
        container('whatbot-postgres', 'database', 'running'),
        container('whatbot-redis', 'cache', 'exited'),
      ]);
...
describe('DockerService.getContainerByService exact-name fallback', () => {
  ...
  // whatbot-managed container name, never a substring (a substring — and especially the empty string —
  ...
  it('does not resolve any container for an empty service name', async () => {
    const { service, getContainer } = withFakeDocker([{ Id: 'abc', Names: ['/whatbot-postgres'] }]);
  ...
  it('does not resolve a container by substring of its name', async () => {
    const { service, getContainer } = withFakeDocker([{ Id: 'abc', Names: ['/whatbot-postgres-primary'] }]);
    // 'postgres' is a substring of 'whatbot-postgres-primary' but not the exact managed name.
    expect(await service.getContainerByService('postgres')).toBeNull();
    expect(getContainer).not.toHaveBeenCalled();
  });

  it('resolves the exact whatbot-<service> container', async () => {
    const { service, getContainer } = withFakeDocker([
      { Id: 'p', Names: ['/whatbot-postgres'] },
      { Id: 'r', Names: ['/whatbot-redis'] },
    ]);
