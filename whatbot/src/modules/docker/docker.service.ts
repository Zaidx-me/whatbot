...
/**
 * The only Docker profiles whatbot manages (and may start/stop/remove). Used to bound teardown so a
...
export class DockerService implements OnModuleInit {
  ...
  /**
   * List all whatbot-related containers
   */
  async listContainers(): Promise<ContainerInfo[]> {
    ...
    try {
      ...
      return containers
        .filter(c => {
          // Filter by whatbot labels or name prefix
          const labels = c.Labels || {};
          return labels['com.whatbot.service'] || c.Names?.some(n => n.startsWith('/whatbot-'));
        })
        .map(c => ({
  ...
  /**
   * Which bundled (whatbot-managed) service containers are currently RUNNING, keyed by the
   * `com.whatbot.service` label (`database` | `cache` | `storage`). Lets the dashboard show the real
  ...
  async getRunningBuiltinServices(): Promise<{ database: boolean; cache: boolean; storage: boolean }> {
    ...
    const isRunning = (svc: string): boolean =>
      containers.some(
        c =>
          c.labels['com.whatbot.service'] === svc && c.labels['com.whatbot.builtin'] === 'true' && c.state === 'running',
      );
  ...
  async getContainerByService(service: string): Promise<Docker.Container | null> {
    ...
    try {
      const containers = await this.docker.listContainers({
        all: true,
        filters: {
          label: [`com.whatbot.service=${service}`],
        },
      });
      ...
      // string, would resolve an arbitrary container). whatbot-managed containers are `whatbot-<service>`.
      const target = `whatbot-${service}`;
  ...
  private getContainerSpec(profile: string): {
    ...
    const specs: Record<string, ReturnType<typeof this.getContainerSpec>> = {
      redis: {
        ...
        volumes: [{ name: 'whatbot_redis-data', path: '/data' }],
        healthcheck: {
        ...
        labels: {
          'com.whatbot.service': 'cache',
          'com.whatbot.builtin': 'true',
        },
      },
      postgres: {
        image: 'postgres:16-alpine',
        name: 'whatbot-postgres',
        alias: 'postgres',
        // Use hardcoded defaults for built-in container (don't inherit SQLite paths)
        env: ['POSTGRES_USER=whatbot', 'POSTGRES_PASSWORD=whatbot', 'POSTGRES_DB=whatbot'],
        volumes: [{ name: 'whatbot_postgres-data', path: '/var/lib/postgresql/data' }],
        healthcheck: {
          test: ['CMD-SHELL', 'pg_isready -U whatbot'],
          interval: 5000000000,
          timeout: 3000000000,
          retries: 5,
        },
        labels: {
          'com.whatbot.service': 'database',
          'com.whatbot.builtin': 'true',
        },
      },
      minio: {
        ...
        volumes: [{ name: 'whatbot_minio-data', path: '/data' }],
        ports: [
