...
describe('WebhookService', () => {
  ...
  describe('dispatch (direct mode)', () => {
    ...
    it('keeps the server-canonical idempotency/delivery ids on the signed body, overriding a tampering plugin', async () => {
      ...
      expect(body.idempotencyKey).toBe(headers['X-Whatbot-Idempotency-Key']);
      expect(body.deliveryId).toBe(headers['X-Whatbot-Delivery-Id']);
  ...
  describe('custom header merge', () => {
    it('drops reserved custom headers so the system headers always win', async () => {
      const webhook = createMockWebhook({
        events: ['message.received'],
        headers: { 'X-Whatbot-Event': 'forged', 'Content-Type': 'text/plain', 'X-Custom': 'ok' },
      });
      (repository.find as jest.Mock).mockResolvedValue([webhook]);
      (repository.update as jest.Mock).mockResolvedValue({ affected: 1 });
      ...
      expect(captured['X-Whatbot-Event']).toBe('message.received'); // system value, not 'forged'
  ...
  describe('generateSignature', () => {
    it('should produce valid HMAC-SHA256 signature', async () => {
      ...
      expect(capturedHeaders['X-Whatbot-Signature']).toMatch(/^sha256=[a-f0-9]{64}$/);
      ...
      expect(capturedHeaders['X-Whatbot-Signature']).toBe(expected);
