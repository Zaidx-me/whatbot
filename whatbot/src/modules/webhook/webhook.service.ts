...
export class WebhookService {
  ...
  async test(sessionId: string, webhookId: string): Promise<{ success: boolean; statusCode?: number; error?: string }> {
    ...
    const testPayload: WebhookPayload = {
      ...
      data: {
        message: 'This is a test webhook from whatbot',
    ...
    const headers: Record<string, string> = {
      ...
      'User-Agent': 'whatbot-Webhook/1.0.0',
      'X-Whatbot-Event': 'test',
      'X-Whatbot-Idempotency-Key': testPayload.idempotencyKey,
      'X-Whatbot-Delivery-Id': testPayload.deliveryId,
      'X-Whatbot-Retry-Count': '0',
    };

    if (webhook.secret) {
      headers['X-Whatbot-Signature'] = this.generateSignature(body, webhook.secret);
    }
  ...
  async dispatch(sessionId: string, event: string, data: Record<string, unknown>): Promise<void> {
    ...
    const tasks = matchingWebhooks.map(async webhook => {
      ...
      // (receivers dedupe on the X-Whatbot-Idempotency-Key header). Re-assert them onto the post-hook
      ...
      const headers: Record<string, string> = {
        ...
        'Content-Type': 'application/json',
        'User-Agent': 'whatbot-Webhook/1.0.0',
        'X-Whatbot-Event': event,
        'X-Whatbot-Idempotency-Key': idempotencyKey,
        'X-Whatbot-Delivery-Id': deliveryId,
        'X-Whatbot-Retry-Count': '0',
      };
      ...
      if (this.queueEnabled && this.webhookQueue) {
        try {
          ...
          if (webhook.secret) {
            headers['X-Whatbot-Signature'] = signature;
          }
          ...
          // same X-Whatbot-Idempotency-Key / X-Whatbot-Delivery-Id, so a conformant receiver dedupes.
          try {
  ...
  private async deliverWebhook(
    ...
    headers['X-Whatbot-Retry-Count'] = String(attempt - 1);

    // Add signature if secret is configured and not already present
    if (webhook.secret && !headers['X-Whatbot-Signature']) {
      headers['X-Whatbot-Signature'] = this.generateSignature(body, webhook.secret);
    }
   ...
   * X-Whatbot-* header) so a webhook config cannot forge the signature/event/idempotency
  ...
  private sanitizeCustomHeaders(custom: Record<string, string> | null | undefined): Record<string, string> {
    ...
    for (const [key, value] of Object.entries(custom ?? {})) {
      if (!/^(content-type|x-whatbot-)/i.test(key)) {
        safe[key] = value;
      }
    }
    return safe;
