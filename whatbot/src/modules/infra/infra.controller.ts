...
interface InfraStatus {
  // `builtIn` reflects whether whatbot's own bundled container is actually running and backing this
...
export class InfraController {
      ...
      if (config.database) {
        ...
        if (config.database.type === 'postgres') {
          if (config.database.builtIn) {
            ...
            updates.DATABASE_USERNAME = 'whatbot';
            updates.DATABASE_PASSWORD = 'whatbot';
            updates.DATABASE_NAME = 'whatbot';
            ...
            updates.DATABASE_NAME = config.database.database || 'whatbot';
          }
  ...
  async requestRestart(@Body() body?: { profiles?: string[]; profilesToRemove?: string[] }): Promise<{
    ...
    if (this.dockerService.isDockerAvailable()) {
      ...
      // Only ever tear down whatbot-managed services. An arbitrary profile name (or the empty string)
