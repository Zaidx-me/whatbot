...
export type ThemePalette = 'whatbot' | 'blue' | 'graphite' | 'indigo' | 'amber' | 'rose' | 'teal';

const THEME_KEY = 'whatbot_theme';
const PALETTE_KEY = 'whatbot_palette';

export const paletteOptions: Array<{ value: ThemePalette; label: string; color: string }> = [
  { value: 'whatbot', label: 'whatbot', color: '#25d366' },
...
export function useTheme() {
  ...
  const [palette, setPaletteState] = useState<ThemePalette>(() => {
    ...
    return isPalette(saved) ? saved : 'whatbot';
  });
