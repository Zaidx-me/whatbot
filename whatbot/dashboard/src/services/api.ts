// API Service Layer for whatbot Dashboard
...
export interface InfraStatus {
  // `builtIn` = whatbot's own bundled container is actually running and backing this service (live),
...
async function request<T>(endpoint: string, options: RequestInit = {}): Promise<T> {
  ...
  const apiKey = sessionStorage.getItem('whatbot_api_key');
  ...
  if (response.status === 401) {
    ...
    sessionStorage.removeItem('whatbot_api_key');
...
export interface CatalogPlugin {
  ...
  license?: string;
  keywords?: string[];
  minwhatbotVersion?: string;
  testedwhatbotVersion?: string;
  homepage?: string;
  download?: string;
  installed: boolean;
